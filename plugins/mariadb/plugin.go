...
func init() {
	plugin.Register(plugin.TypeMariaDB, func() plugin.DBPlugin { return &mariaPlugin{} })
}

type mariaPlugin struct {
	db  *sql.DB
	cfg plugin.Config
}

func (p *mariaPlugin) Type() plugin.DBType { return plugin.TypeMariaDB }

func (p *mariaPlugin) Connect(ctx context.Context, cfg plugin.Config) error {
...
func (p *mariaPlugin) Query(ctx context.Context, q string) (*plugin.Result, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mariadb: columns: %w", err)
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(ptrs...); err != nil {
			return nil, fmt.Errorf("mariadb: scan: %w", err)
		}
...
func (p *mariaPlugin) Tables(ctx context.Context) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mariadb: list tables: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t); err != nil {
			return nil, fmt.Errorf("mariadb: scan table: %w", err)
		}
...
func (p *mariaPlugin) Databases(ctx context.Context) ([]string, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&db); err != nil {
			return nil, fmt.Errorf("mariadb: scan db: %w", err)
		}
		dbs = append(dbs, db)
	}
	return dbs, nil
